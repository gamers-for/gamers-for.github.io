---
title: "シャープマーカーのギア構成と立ち回り｜シャプマ【スプラトゥーン3】"
linkTitle: "シャープマーカー"
weight: 10
date: 2026-02-13
categories: ["シューター"]
tags: ["スプラトゥーン3", "シューター", "シャープマーカー"]
description: "スプラトゥーン3のシャープマーカーの性能評価・立ち回り解説。サブクイックボム、スペシャルカニタンクの使い方やおすすめギアパワーを紹介。"
---

<p class="a-paragraph"><img alt="シャープマーカー" class="a-img img-placeholder" loading="lazy" src="/images/games/splatoon3/placeholder-icon.png" width="440"/></p>
<p class="a-paragraph">スプラトゥーン3(スプラ3)における、シャープマーカー（シャプマ）を詳しくまとめていくよ。</p>
<p></p>
<div class="a-outline">
<p><svg aria-hidden="true" class="svg-inline--fa fa-list-ul fa-w-16" data-icon="list-ul" data-prefix="fas" focusable="false" role="img" viewbox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="M48 48a48 48 0 1 0 48 48 48 48 0 0 0-48-48zm0 160a48 48 0 1 0 48 48 48 48 0 0 0-48-48zm0 160a48 48 0 1 0 48 48 48 48 0 0 0-48-48zm448 16H176a16 16 0 0 0-16 16v32a16 16 0 0 0 16 16h320a16 16 0 0 0 16-16v-32a16 16 0 0 0-16-16zm0-320H176a16 16 0 0 0-16 16v32a16 16 0 0 0 16 16h320a16 16 0 0 0 16-16V80a16 16 0 0 0-16-16zm0 160H176a16 16 0 0 0-16 16v32a16 16 0 0 0 16 16h320a16 16 0 0 0 16-16v-32a16 16 0 0 0-16-16z"></path></svg>目次</p>
<ul>
<li><a class="track_nier" href="#hl_1">評価と役割</a></li>
<li><a class="track_nier" href="#hl_2">イチオシギア構成</a></li>
<li><a class="track_nier" href="#hl_3">動き方と使い方</a></li>
<li><a class="track_nier" href="#hl_4">対策</a></li>
<li><a class="track_nier" href="#hl_5">性能と射程比較</a></li>
<li><a class="track_nier" href="#hl_6">アップデート調整内容</a></li>
<li><a class="track_nier" href="#hl_7">同名武器と違い</a></li>
<li><a class="track_nier" href="#hl_8">関連記事</a></li>
</ul>
</div>


<h2 class="a-header--2" id="hl_1">シャープマーカーの評価と役割</h2>
<table class="a-table a-table table--fixed a-table" style="">
<tr>
<td class="center" colspan="2" rowspan="4" width="50%">
<img alt="シャープマーカー画像" class="a-img" height="80" loading="lazy" src="/images/games/splatoon3/weapons/splash-o-matic.png" width="80"/><br/>シャープマーカー
</td>
<th colspan="2" width="50%">総合評価</th>
</tr>
<tr>
<td class="center" colspan="2">
<img alt="X" class="a-img" loading="lazy" src="/images/games/splatoon3/tiers/x.webp" width="40"/>
</td>
</tr>
<tr>
<th colspan="2">サブ・スペ</th>
</tr>
<tr>
<td colspan="2">
<div class="align">
<a class="a-link" href="#"><img alt="クイックボム画像" class="a-img" loading="lazy" src="/images/games/splatoon3/subs/burst-bomb.png" width="30"/>クイックボム</a>
</div>
<div class="align">
<a class="a-link" href="#"><img alt="カニタンク画像" class="a-img" loading="lazy" src="/images/games/splatoon3/specials/crab-tank.png" width="30"/>カニタンク</a>
</div>
</td>
</tr>
<tr>
<th width="23%">塗り</th>
<td class="center" width="27%">
<div class="align"><img alt="星5の画像" class="a-img" loading="lazy" src="/images/games/splatoon3/stars/star5.png" width="90"/></div>
</td>
<th width="23%">扱いやすさ</th>
<td class="center" width="27%">
<div class="align"><img alt="星4の画像" class="a-img" loading="lazy" src="/images/games/splatoon3/stars/star4.png" width="90"/></div>
</td>
</tr>
<tr>
<th>キル</th>
<td class="center">
<div class="align"><img alt="星4の画像" class="a-img" loading="lazy" src="/images/games/splatoon3/stars/star4.png" width="90"/></div>
</td>
<th>防御・生存</th>
<td class="center">
<div class="align"><img alt="星4の画像" class="a-img" loading="lazy" src="/images/games/splatoon3/stars/star4.png" width="90"/></div>
</td>
</tr>
<tr>
<th>アシスト</th>
<td class="center">
<div class="align"><img alt="星5の画像" class="a-img" loading="lazy" src="/images/games/splatoon3/stars/star5.png" width="90"/></div>
</td>
<th>打開力</th>
<td class="center">
<div class="align"><img alt="星5の画像" class="a-img" loading="lazy" src="/images/games/splatoon3/stars/star5.png" width="90"/></div>
</td>
</tr>
</table>
<h3 class="a-header--3" id="hm_1">ルール別評価</h3>
<table class="a-table a-table table--fixed a-table" style="">
<tr>
<th width="20%"><a class="a-link" href="/games/splatoon3/tier-nawabari/"><img alt="スプラトゥーン3のナワバリバトル" class="a-img" height="25" loading="lazy" src="/images/games/splatoon3/rules/turf-war.png" width="25"/>ナワバリ</a></th>
<th width="20%"><a class="a-link" href="/games/splatoon3/tier-area/"><img alt="スプラトゥーン3のガチエリア" class="a-img" height="25" loading="lazy" src="/images/games/splatoon3/rules/splat-zones.png" width="25"/>エリア</a></th>
<th width="20%"><a class="a-link" href="/games/splatoon3/tier-yagura/"><img alt="スプラトゥーン3のガチヤグラ" class="a-img" height="25" loading="lazy" src="/images/games/splatoon3/rules/tower-control.png" width="25"/>ヤグラ</a></th>
<th width="20%"><a class="a-link" href="/games/splatoon3/tier-hoko/"><img alt="スプラトゥーン3のガチホコバトル" class="a-img" height="25" loading="lazy" src="/images/games/splatoon3/rules/rainmaker.png" width="25"/>ホコ</a></th>
<th width="20%"><a class="a-link" href="/games/splatoon3/tier-asari/"><img alt="スプラトゥーン3のガチアサリ" class="a-img" height="25" loading="lazy" src="/images/games/splatoon3/rules/clam-blitz.png" width="25"/>アサリ</a></th>
</tr>
<tr>
<td class="center">
<img alt="X" class="a-img" loading="lazy" src="/images/games/splatoon3/tiers/x.webp" width="25"/>
</td>
<td class="center">
<img alt="X" class="a-img" loading="lazy" src="/images/games/splatoon3/tiers/x.webp" width="25"/>
</td>
<td class="center">
<img alt="X" class="a-img" loading="lazy" src="/images/games/splatoon3/tiers/x.webp" width="25"/>
</td>
<td class="center">
<img alt="X" class="a-img" loading="lazy" src="/images/games/splatoon3/tiers/x.webp" width="25"/>
</td>
<td class="center">
<img alt="X" class="a-img" loading="lazy" src="/images/games/splatoon3/tiers/x.webp" width="25"/>
</td>
</tr>
</table>
<p class="a-paragraph"><a class="a-btn" href="/games/splatoon3/tier-list/"><span>▶︎最強武器ランキングはこちら！</span></a></p>


<h3 class="a-header--3" id="hm_2">なんでもできる器用な武器</h3>
<table class="a-table a-table" style="">
<tr>
<th>強い点</th>
</tr>
<tr>
<td width="100%">
<div class="align">
<img alt="強い点" class="a-img" loading="lazy" src="/images/games/splatoon3/markers/check.png" width="16"/><b class="a-bold">メイン射撃の射程が全くブレることなく塗りも強い</b>
</div>
<hr class="a-table__line"/>
<div class="align">
<img alt="強い点" class="a-img" loading="lazy" src="/images/games/splatoon3/markers/check.png" width="16"/><b class="a-bold">クイボがカバーやキルに向いている</b>
</div>
<hr class="a-table__line"/>
<div class="align">
<img alt="強い点" class="a-img" loading="lazy" src="/images/games/splatoon3/markers/check.png" width="16"/><b class="a-bold">カニタンクによるキルが取りやすく強力</b>
</div>
</td>
</tr>
<tr>
<th>弱い点</th>
</tr>
<tr>
<td width="100%">
<div class="align">
<img alt="弱い点" class="a-img" loading="lazy" src="/images/games/splatoon3/markers/cross.png" width="16"/><b class="a-bold">メインウェポンの射程が短め</b>
</div>
</td>
</tr>
</table>
<p class="a-paragraph">シャープマーカーは、メインによる塗りやブレがないことによるキル、クイボによるトドメやカバー足元塗り、カニタンクによるキルや前線キープなどオールラウンドに活躍できる万能武器という性能の武器だよ。初心者にも玄人にもイチオシできるよ。</p>
<h2 class="a-header--2" id="hl_2">シャープマーカーのおすすめギア構成</h2>
<h3 class="a-header--3" id="hm_3">迷ったらコレがおすすめ！</h3>
<table class="a-table a-table" style="">
<tr>
<th width="34%">メイン</th>
<th width="22%">サブ①</th>
<th width="22%">サブ②</th>
<th width="22%">サブ③</th>
</tr>
<tr>
<td class="center" width="34%">
<a class="a-link" href="#"><img alt="イカダッシュ速度アップ画像" class="a-img" height="40" loading="lazy" src="/images/games/splatoon3/gear-powers/swim-speed-up.png" width="40"/>イカ速</a>
</td>
<td class="center" width="22%">
<a class="a-link" href="#"><img alt="イカダッシュ速度アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/swim-speed-up.png" width="30"/>イカ速</a>
</td>
<td class="center" width="22%">
<a class="a-link" href="#"><img alt="サブ性能アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/sub-power-up.png" width="30"/>サブ性能</a>
</td>
<td class="center" width="22%">
<a class="a-link" href="#"><img alt="サブ性能アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/sub-power-up.png" width="30"/>サブ性能</a>
</td>
</tr>
<tr>
<td class="center">
<a class="a-link" href="#"><img alt="イカニンジャ画像" class="a-img" height="40" loading="lazy" src="/images/games/splatoon3/gear-powers/ninja-squid.png" width="40"/>イカニン</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="インク回復力アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/ink-recovery-up.png" width="30"/>インク回復</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="インク回復力アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/ink-recovery-up.png" width="30"/>インク回復</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="スペシャル減少量ダウン画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/special-saver.png" width="30"/>スペ減</a>
</td>
</tr>
<tr>
<td class="center">
<a class="a-link" href="#"><img alt="ステルスジャンプ画像" class="a-img" height="40" loading="lazy" src="/images/games/splatoon3/gear-powers/stealth-jump.png" width="40"/>ステジャン</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="サブ影響軽減画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/sub-resistance-up.png" width="30"/>サブ軽減</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="相手インク影響軽減画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/ink-resistance-up.png" width="30"/>安全靴</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="スーパージャンプ時間短縮画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/quick-super-jump.png" width="30"/>ジャン短</a>
</td>
</tr>
</table>
<p class="a-paragraph">シャープマーカーの基本となるギア構成だよ。イカニンジャを装備するとメイン射撃のキルをしやすくし、サブ性能アップでクイボの飛距離を伸ばしている。メインギアは全て必須級ですが、サブは好みでインク回復力アップなどを積んでも良いなんだ。</p>


<h3 class="a-header--3" id="hm_4">ガチエリアはスペ増対物がおすすめ</h3>
<table class="a-table a-table" style="">
<tr>
<th width="34%">メイン</th>
<th width="22%">サブ①</th>
<th width="22%">サブ②</th>
<th width="22%">サブ③</th>
</tr>
<tr>
<td class="center" width="34%">
<a class="a-link" href="#"><img alt="スペシャル増加量アップ画像" class="a-img" height="40" loading="lazy" src="/images/games/splatoon3/gear-powers/special-charge-up.png" width="40"/>スペ増</a>
</td>
<td class="center" width="22%">
<a class="a-link" href="#"><img alt="スペシャル増加量アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/special-charge-up.png" width="30"/>スペ増</a>
</td>
<td class="center" width="22%">
<a class="a-link" href="#"><img alt="サブ性能アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/sub-power-up.png" width="30"/>サブ性能</a>
</td>
<td class="center" width="22%">
<a class="a-link" href="#"><img alt="サブ性能アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/sub-power-up.png" width="30"/>サブ性能</a>
</td>
</tr>
<tr>
<td class="center">
<a class="a-link" href="#"><img alt="スペシャル増加量アップ画像" class="a-img" height="40" loading="lazy" src="/images/games/splatoon3/gear-powers/special-charge-up.png" width="40"/>スペ増</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="インク回復力アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/ink-recovery-up.png" width="30"/>インク回復</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="インク回復力アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/ink-recovery-up.png" width="30"/>インク回復</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="相手インク影響軽減画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/ink-resistance-up.png" width="30"/>安全靴</a>
</td>
</tr>
<tr>
<td class="center">
<a class="a-link" href="#"><img alt="対物攻撃力アップ画像" class="a-img" height="40" loading="lazy" src="/images/games/splatoon3/gear-powers/object-shredder.png" width="40"/>対物</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="スペシャル減少量ダウン画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/special-saver.png" width="30"/>スペ減</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="スペシャル減少量ダウン画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/special-saver.png" width="30"/>スペ減</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="スーパージャンプ時間短縮画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/quick-super-jump.png" width="30"/>ジャン短</a>
</td>
</tr>
</table>
<p class="a-paragraph">スペシャル増加量アップでカニタンクを回しまくる構成が推奨だ。。ガチエリアではカニタンクがなかでも頼もしいからだ加えて、必ずと言っていいほど敵にもシャープマーカーが居るので、カニタンク対面を意識して対物攻撃力アップを付けましょう。</p>
<p class="a-paragraph">ガチエリア以外のルールでも、カニタンクを回したい方はこちらの構成で問題ない。</p>
<h3 class="a-header--3" id="hm_5">おすすめギアパワー一覧と付きやすいブランド</h3>
<table class="a-table a-table" style="">
<tr>
<th width="30%">ギアパワー</th>
<th width="70%">おすすめ理由／付きやすいブランド</th>
</tr>
<tr>
<td class="center"><a class="a-link" href="#"><img alt="インク効率アップ（サブ）画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/ink-saver-sub.png" width="30"/>サブ効率</a></td>
<td>クイックボムを連投しやすくなるよ。<hr class="a-table__line"/>
<div class="align"><a class="a-link" href="#"><img alt="ホッコリー画像" class="a-img" height="20" loading="lazy" src="/images/games/splatoon3/brands/hokkori.png" width="20"/>ホッコリー</a></div>
<div class="align"><a class="a-link" href="#"><img alt="アナアキ画像" class="a-img" height="20" loading="lazy" src="/images/games/splatoon3/brands/anaki.png" width="20"/>アナアキ</a></div>
</td>
</tr>
<tr>
<td class="center"><a class="a-link" href="#"><img alt="インク回復力アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/ink-recovery-up.png" width="30"/>インク回復</a></td>
<td>インクが足りないと感じる場合に積むと良いだよ。クイックボムはサブ効率の乗り具合が良くないので、ギア効率的にインク回復の方がイチオシ。<hr class="a-table__line"/>
<div class="align"><a class="a-link" href="#"><img alt="アロメ画像" class="a-img" height="20" loading="lazy" src="/images/games/splatoon3/brands/zekko.png" width="20"/>アロメ</a></div>
</td>
</tr>
<tr>
<td class="center"><a class="a-link" href="#"><img alt="イカダッシュ速度アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/swim-speed-up.png" width="30"/>イカ速</a></td>
<td>前線で接敵する際や前線から引く際など、前線武器や短射程武器必須級の万能なギアだ。<hr class="a-table__line"/>
<div class="align"><a class="a-link" href="#"><img alt="クラーゲス画像" class="a-img" height="20" loading="lazy" src="/images/games/splatoon3/brands/krak-on.png" width="20"/>クラーゲス</a></div>
</td>
</tr>
<tr>
<td class="center"><a class="a-link" href="#"><img alt="スペシャル増加量アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/special-charge-up.png" width="30"/>スペ増</a></td>
<td>前線を押し上げたり維持したりできるカニタンクの回転を上げます。<hr class="a-table__line"/>
<div class="align"><a class="a-link" href="#"><img alt="ヤコ画像" class="a-img" height="20" loading="lazy" src="/images/games/splatoon3/brands/inkline.png" width="20"/>ヤコ</a></div>
</td>
</tr>
<tr>
<td class="center"><a class="a-link" href="#"><img alt="スペシャル減少量ダウン画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/special-saver.png" width="30"/>スペ減</a></td>
<td>基本は前線で立ち回る武器なので倒されやすいですが、このギアをつけることによりスペシャルの回転を上げられるよ。<hr class="a-table__line"/>
<div class="align"><a class="a-link" href="#"><img alt="エゾッコ画像" class="a-img" height="20" loading="lazy" src="/images/games/splatoon3/brands/ezokko.png" width="20"/>エゾッコ</a></div>
<div class="align"><a class="a-link" href="#"><img alt="エゾッコリー画像" class="a-img" height="20" loading="lazy" src="/images/games/splatoon3/brands/ezokkori.png" width="20"/>エゾッコリー</a></div>
</td>
</tr>
<tr>
<td class="center"><a class="a-link" href="#"><img alt="スーパージャンプ時間短縮画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/quick-super-jump.png" width="30"/>ジャン短</a></td>
<td>リスポーン高速化と緊急離脱力の底上げ、両方のメリットを得られるのが優秀を上げてくれるよ。<hr class="a-table__line"/>
<div class="align"><a class="a-link" href="#"><img alt="アイロニック画像" class="a-img" height="20" loading="lazy" src="/images/games/splatoon3/brands/annaki.png" width="20"/>アイロニック</a></div>
</td>
</tr>
<tr>
<td class="center"><a class="a-link" href="#"><img alt="サブ性能アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/sub-power-up.png" width="30"/>サブ性能</a></td>
<td>クイックボムの飛距離が伸びます。カバーや追撃がしやすくなる他、スピードも上がるので相手に当てやすくなるよ。<hr class="a-table__line"/>
<div class="align"><a class="a-link" href="#"><img alt="エンペリー画像" class="a-img" height="20" loading="lazy" src="/images/games/splatoon3/brands/enperry.png" width="20"/>エンペリー</a></div>
</td>
</tr>
<tr>
<td class="center"><a class="a-link" href="#"><img alt="相手インク影響軽減画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/ink-resistance-up.png" width="30"/>安全靴</a></td>
<td>相手インクを踏みやすい前衛での動きが安定する。<hr class="a-table__line"/>
<div class="align"><a class="a-link" href="#"><img alt="バトロイカ画像" class="a-img" height="20" loading="lazy" src="/images/games/splatoon3/brands/splash-mob.png" width="20"/>バトロイカ</a></div>
</td>
</tr>
<tr>
<td class="center"><a class="a-link" href="#"><img alt="カムバック画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/comeback.png" width="30"/>カムバ</a></td>
<td>※<span class="a-red">アタマのメインギアパワー限定</span><hr class="a-table__line"/>前線で立ち回る武器なので倒されやすいですが、倒された後の巻き返しをしやすくしてくれるよ。<hr class="a-table__line"/>なし</td>
</tr>
<tr>
<td class="center"><a class="a-link" href="#"><img alt="ステルスジャンプ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/stealth-jump.png" width="30"/>ステジャン</a></td>
<td>※<span class="a-red">クツのメインギアパワー限定</span><hr class="a-table__line"/>相手にスーパージャンプの着地を見られにくくなることにより、前線に早く復帰しやすくなるよ。<hr class="a-table__line"/>なし</td>
</tr>
<tr>
<td class="center"><a class="a-link" href="#"><img alt="対物攻撃力アップ画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/gear-powers/object-shredder.png" width="30"/>対物</a></td>
<td>※<span class="a-red">クツのメインギアパワー限定</span><hr class="a-table__line"/>カニタンク対面に強くなる。そのうえ、ナイスダマやホップソナーを壊す際にも有効だ。<hr class="a-table__line"/>なし</td>
</tr>
</table>

<h2 class="a-header--2" id="hl_3">シャープマーカーの立ち回りと使い方</h2>
<table class="a-table a-table" style="">
<tr>
<td width="100%">
<div class="align">
<img alt="ポイント" class="a-img" loading="lazy" src="/images/games/splatoon3/markers/check.png" width="16"/><b class="a-bold">イカニンジャを駆使して敵を倒す</b>
</div>
<hr class="a-table__line"/>
<div class="align">
<img alt="ポイント" class="a-img" loading="lazy" src="/images/games/splatoon3/markers/check.png" width="16"/><b class="a-bold">クイボでカバーをすることを意識する</b>
</div>
<hr class="a-table__line"/>
<div class="align">
<img alt="ポイント" class="a-img" loading="lazy" src="/images/games/splatoon3/markers/check.png" width="16"/><b class="a-bold">暇さえされば塗りをしてカニタンクの回転を意識する</b>
</div>
<hr class="a-table__line"/>
<div class="align">
<img alt="NG行動" class="a-img" loading="lazy" src="/images/games/splatoon3/markers/cross.png" width="16"/><b class="a-bold">無理なスーパージャンプをしない</b>
</div>
</td>
</tr>
</table>
<h3 class="a-header--3" id="hm_6">イカニンジャを駆使してキルを取る</h3>
<p class="a-paragraph">シャープマーカーは、メインウェポンの射程が短めのためイカニンジャを駆使して相手に近づきキルを取りましょう。ただ注意として味方インクがあまり塗り広がっていない場合のイカニンジャはあまり強くないため、そのときは塗りをしてカニタンクを溜めるなどイカニンジャが弱い状況の見極めとそのときの無理をしないプレイも同時に覚えましょう。</p>
<h3 class="a-header--3" id="hm_7">クイボでカバーをすることを意識する</h3>
<p class="a-paragraph"><img alt="クイックボムから対面に入る" class="a-img img-placeholder" loading="lazy" src="/images/games/splatoon3/placeholder-icon.png" width="440"/><br/>シャープマーカーのサブはクイックボムのためメインで倒しきることができなかった敵や味方のカバーに使いましょう。サブ性能アップ0.2がちょうど自分の対面にも味方のカバーにも入りやすい程度のクイボの飛距離のためイチオシ。</p>
<h3 class="a-header--3" id="hm_8">カニタンクの回転を意識する</h3>
<p class="a-paragraph">シャープマーカーはスペシャルのカニタンクがかなり優秀だよ。なので、逆転や相手に位置がバレている際は無理してメインとサブで突っ込むのではなく塗りをしてしっかりカニタンクを溜めましょう。カニタンクを使えさえすればボムの直撃+爆風と射撃一発で倒しきれるので戦況をひっくり返せます。</p>
<p class="a-paragraph"><a class="a-btn" href="#"><span>▶︎カニタンクの詳しい使い方を見る</span></a></p>

<h3 class="a-header--3" id="hm_9">無理なスーパージャンプをしない</h3>
<p class="a-paragraph">シャープマーカーはステルスジャンプを付けている場合でも、無理なジャンプはしないようにしよう。デスをした場合はすぐにマップを見て前線をキープできる味方が生きていること、そしてその後ろに中衛や後衛がいることなどを確認して安全そうな場合のみ中衛や後衛の味方に飛ぶようにしよう。すべての武器に言えることによりすが、なかでもスペシャルが強いシャープマーカーの連続デスは痛いなんだ。</p>
<h2 class="a-header--2" id="hl_4">シャープマーカーの対策</h2>
<table class="a-table a-table" style="">
<tr>
<td width="100%">
<div class="align">
<img alt="ポイント" class="a-img" loading="lazy" src="/images/games/splatoon3/markers/check.png" width="16"/><b class="a-bold">射程範囲外から攻撃する</b>
</div>
<hr class="a-table__line"/>
<div class="align">
<img alt="ポイント" class="a-img" loading="lazy" src="/images/games/splatoon3/markers/check.png" width="16"/><b class="a-bold">スペシャルの溜まりを意識しておく</b>
</div>
</td>
</tr>
</table>
<h3 class="a-header--3" id="hm_10">射程範囲外から攻撃する</h3>
<p class="a-paragraph">シャープマーカーは、メイン射撃の射程が長くないためクイボにだけ気をつけてできるだけ遠くから戦うようにしよう。イカニンジャを活かして近づかれる場合があるため、相手がイカニンジャを積んでいるなら自分の周囲の塗りは徹底するかつ音をよく聞いて接近に気づくようにしよう。</p>
<h3 class="a-header--3" id="hm_11">スペシャルの溜まりを意識しておく</h3>
<p class="a-paragraph">シャープマーカーはカニタンクがパワフルな武器のため、メイン射撃の届く距離が短いと油断していると突然カニタンクを吐かれて射程がかなり伸びます。とりわけ後衛武器は突然デスをするリスクを下げるためにもシャープマーカーのスペシャルの溜まりは常に意識しておくと良いなんだ。</p>
<h2 class="a-header--2" id="hl_5">シャープマーカーの性能と射程比較</h2>
<h3 class="a-header--3" id="hm_12">性能</h3>
<table class="a-table a-table a-table" style="">
<tr>
<th width="40%">ブキ種</th>
<td class="center" width="60%">
<a class="a-link" href="#">シューター</a>
</td>
</tr>
<tr>
<th>解放ランク</th>
<td class="center">16</td>
</tr>
<tr>
<th>攻撃力</th>
<td class="center">
最大：28.0

</td>
</tr>
<tr>
<th>確定数</th>
<td class="center">4</td>
</tr>
<tr>
<th>射程<br/>(試し撃ちライン)</th>
<td class="center">最大：約2.4<br/>有効：約2.0</td>
</tr>
<tr>
<th>サブ <hr class="a-table__line"/>スペシャル</th>
<td class="center">
<a class="a-link" href="#">クイックボム</a>
<hr class="a-table__line"/>
<a class="a-link" href="#">カニタンク</a>
</td>
</tr>
<tr>
<th>スペシャル必要pt</th>
<td class="center">200</td>
</tr>
</table>

<h3 class="a-header--3" id="hm_13">最大ダメージの射程比較</h3>
<table class="a-table a-table a-table" style="">
<tr>
<th width="37%"><div class="align-left">
<div class="align"><a class="a-link" href="/games/splatoon3/weapons/ボールドマーカー/"><img alt="ボールドマーカー画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/weapons/sploosh-o-matic.png" width="30"/>ボールド</a></div>
</div></th>
<td width="63%"><div class="a-graph_wrap">
<div class="a-graph"><div class="a-bar" style="width: 18%"></div></div>
<div class="a-label">1.2</div>
</div></td>
</tr>
<tr>
<th width="37%"><div class="align-left">
<div class="align"><a class="a-link" href="/games/splatoon3/weapons/スプラローラー/"><img alt="スプラローラー画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/weapons/splat-roller.png" width="30"/>スプラローラー</a></div>
</div></th>
<td width="63%"><div class="a-graph_wrap">
<div class="a-graph"><div class="a-bar" style="width: 23%"></div></div>
<div class="a-label">1.5</div>
</div></td>
</tr>
<tr>
<th width="37%"><div class="align-left">
<div class="align"><a class="a-link" href="/games/splatoon3/weapons/スパッタリー/"><img alt="スパッタリー画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/weapons/dapple-dualies.png" width="30"/>赤スパ</a></div>
</div></th>
<td width="63%"><div class="a-graph_wrap">
<div class="a-graph"><div class="a-bar" style="width: 23%"></div></div>
<div class="a-label">1.5</div>
</div></td>
</tr>
<tr>
<th width="37%"><div class="align-left">
<div class="align"><b class="a-bold"><img alt="シャープマーカー画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/weapons/splash-o-matic.png" width="30"/>シャプマ</b></div>
</div></th>
<td width="63%"><div class="a-graph_wrap">
<div class="a-graph"><div class="a-bar" style="width: 30%"></div></div>
<div class="a-label">2.0</div>
</div></td>
</tr>
<tr>
<th width="37%"><div class="align-left">
<div class="align"><a class="a-link" href="/games/splatoon3/weapons/わかばシューター/"><img alt="わかばシューター画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/weapons/splattershot-jr.png" width="30"/>わかば</a></div>
</div></th>
<td width="63%"><div class="a-graph_wrap">
<div class="a-graph"><div class="a-bar" style="width: 30%"></div></div>
<div class="a-label">2.0</div>
</div></td>
</tr>
<tr>
<th width="37%"><div class="align-left">
<div class="align"><a class="a-link" href="/games/splatoon3/weapons/スプラシューター/"><img alt="スプラシューター画像" class="a-img" height="30" loading="lazy" src="/images/games/splatoon3/weapons/splattershot.png" width="30"/>スシ</a></div>
</div></th>
<td width="63%"><div class="a-graph_wrap">
<div class="a-graph"><div class="a-bar" style="width: 36%"></div></div>
<div class="a-label">2.4</div>
</div></td>
</tr>
</table>
<p class="a-footnote">※現環境に多く、同程度の射程ブキで比較しています。</p>

<h2 class="a-header--2" id="hl_6">シャープマーカーのアップデート調整内容</h2>
<h3 class="a-header--3" id="hm_14">2023年5月31日のアップデート調整内容</h3>
<h4 class="a-header--4" id="hs_1">メインウェポンの調整</h4>
<table class="a-table a-table" style="">
<tr>
<th class="center" colspan="5">Ver.4.0.0</th>
</tr>
<tr>
<td width="20%"><b class="a-bold">・着弾点前方への塗りの伸びを、これまでより約10%短くしました。</b></td>
</tr>
</table>
<p class="a-paragraph"> </p>
<div class="a-accordion">
<button class="a-accordion__btn js-accordion-toggle">過去の調整内容</button>
<div class="a-accordion__content">
<h3 class="a-header--3" id="hm_15">2023年3月31日のアップデート調整内容</h3>
<h4 class="a-header--4" id="hs_2">サブウェポンの調整</h4>
<table class="a-table a-table" style="">
<tr>
<th class="center" colspan="5">Ver.3.1.0</th>
</tr>
<tr>
<td class="center" width="20%">
<a class="a-link" href="#"><img alt="クイックボム画像" class="a-img" loading="lazy" src="/images/games/splatoon3/subs/burst-bomb.png" width="50"/>クイックボム</a>
</td>
</tr>
<tr>
<th class="center" colspan="5">調整された内容</th>
</tr>
<tr>
<td>
<b class="a-bold">【弱体化内容】</b>
<br/>・<b class="a-bold">ギアパワー「インク効率アップ(サブ)」を装着していない場合のインクの燃費量を、標準インクタンクの40%から45%に増やしました。</b>
<br/>‐最大までギアパワーを装着している場合のインクの燃費量はこれまでと変わりないよ。</td>
</tr>
</table>
<h4 class="a-header--4" id="hs_3">スペシャルの調整内容</h4>
<table class="a-table a-table" style="">
<tr>
<th class="center" colspan="5">Ver.3.1.0</th>
</tr>
<tr>
<td class="center" width="20%">
<a class="a-link" href="#"><img alt="カニタンク画像" class="a-img" loading="lazy" src="/images/games/splatoon3/specials/crab-tank.png" width="50"/>カニタンク</a>
</td>
</tr>
<tr>
<th class="center" colspan="5">調整された内容</th>
</tr>
<tr>
<td>
<b class="a-bold">【強化内容】</b>
<br/>・<b class="a-bold">移動形態から攻撃形態に戻ったとき、約20/60秒間攻撃が行えないようにしました。</b>
<br/>・<b class="a-bold">スペシャルが終了したとき、次に行動できるまでの時間を延長しました。</b>
</td>
</tr>
</table>
<h3 class="a-header--3" id="hm_16">2023年2月28日のアップデート調整内容</h3>
<h4 class="a-header--4" id="hs_4">サブウェポンの調整</h4>
<table class="a-table a-table" style="">
<tr>
<th class="center" colspan="5">Ver.3.0.0</th>
</tr>
<tr>
<td class="center" width="20%">
<a class="a-link" href="#"><img alt="クイックボム画像" class="a-img" loading="lazy" src="/images/games/splatoon3/subs/burst-bomb.png" width="50"/>クイックボム</a>
</td>
</tr>
<tr>
<th class="center" colspan="5">調整された内容</th>
</tr>
<tr>
<td>
<b class="a-bold">【弱体化内容】</b>
<br/><b class="a-bold">・使用後にインクが回復し始めるまでの時間を、約10/60秒間延長しました。</b>
</td>
</tr>
<tr>
</tr>
<tr>
</tr>
</table>
<h3 class="a-header--3" id="hm_17">2023年1月18日のアップデート調整内容</h3>
<h4 class="a-header--4" id="hs_5">スペシャルの調整</h4>
<table class="a-table a-table" style="">
<tr>
<th class="center" colspan="5">Ver.2.1.0</th>
</tr>
<tr>
<td class="center" width="20%">
<a class="a-link" href="#"><img alt="カニタンク画像" class="a-img" loading="lazy" src="/images/games/splatoon3/specials/crab-tank.png" width="50"/>カニタンク</a>
</td>
</tr>
<tr>
<th class="center" colspan="5">調整された内容</th>
</tr>
<tr>
<td>
<b class="a-bold">【弱体化内容】</b>
<br/><b class="a-bold">・「スペシャル性能アップ」のギアパワーを使用していない場合の継続時間を、約9秒間から約8秒間に短くしました。</b>
<br/><b class="a-bold">【変更内容】</b>
<br/><b class="a-bold">・「スペシャル性能アップ」のギアパワーによる継続時間の延長率を増やしました。</b>
<br/>これらの変更により、「スペシャル性能アップ」のギアパワーを最大まで使用している場合の継続時間は、これまでと同じになるよ。</td>
</tr>
<tr>
</tr>
<tr>
</tr>
</table>
<p class="a-paragraph"></p></div>
</div>
<h2 class="a-header--2" id="hl_7">シャープマーカーの同名武器と違い</h2>
<h3 class="a-header--3" id="hm_18">サブとスペシャルが違うだけ</h3>
<table class="a-table a-table a-table" style="">
<tr>
<th width="40%">武器</th>
<th width="30%">サブ</th>
<th width="30%">スペシャル</th>
</tr>
<tr>
<td class="center">
<b class="a-bold"><img alt="シャープマーカー画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/splash-o-matic.png" width="50"/>シャープマーカー</b>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="クイックボム画像" class="a-img" loading="lazy" src="/images/games/splatoon3/subs/burst-bomb.png" width="50"/>クイックボム</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="カニタンク画像" class="a-img" loading="lazy" src="/images/games/splatoon3/specials/crab-tank.png" width="50"/>カニタンク</a>
</td>
</tr>
<tr>
<td class="center">
<a class="a-link" href="/games/splatoon3/weapons/シャープマーカーネオ/"><img alt="シャープマーカーネオ画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/neo-splash-o-matic.png" width="50"/>シャープマーカーネオ</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="キューバンボム画像" class="a-img" loading="lazy" src="/images/games/splatoon3/subs/suction-bomb.png" width="50"/>キューバンボム</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="トリプルトルネード画像" class="a-img" loading="lazy" src="/images/games/splatoon3/specials/triple-inkstrike.png" width="50"/>トリプルトルネード</a>
</td>
</tr>
<tr>
<td class="center">
<img alt="シャープマーカーGECK画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/splash-o-matic-geck.png" width="50"/>シャープマーカーGECK


</td>
<td class="center">
<a class="a-link" href="#"><img alt="ポイズンミスト画像" class="a-img" loading="lazy" src="/images/games/splatoon3/subs/toxic-mist.png" width="50"/>ポイズンミスト</a>
</td>
<td class="center">
<a class="a-link" href="#"><img alt="アメフラシ画像" class="a-img" loading="lazy" src="/images/games/splatoon3/specials/ink-storm.png" width="50"/>アメフラシ</a>
</td>
</tr>
</table>
<p class="a-paragraph">亜種ブキはサブ・スペシャルだけが別物で、メインウェポンのスペックは一緒。マッチするルールやサブスペの好みで選べばOKましょう。</p>
<h2 class="a-header--2" id="hl_8">関連記事</h2>
<h3 class="a-header--3" id="hm_19">新武器の評価一覧</h3>
<table class="a-table a-table table--fixed a-table" style="">
<tr>
<td class="center" colspan="3" width="100%"><a class="a-link" href="#"><img alt="新ブキ一覧" class="a-img img-placeholder" loading="lazy" src="/images/games/splatoon3/placeholder-icon.png" width="440"/>▶︎新武器のサブスペ・評価一覧</a></td>
</tr>
<tr>
<th class="center" colspan="3" width="100%">新ブキ</th>
</tr>
<tr>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="フルイドⅤ画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/reef-lux-450.png" width="50"/>フルイド</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="フルイドⅤカスタム画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/reef-lux-450-deco.png" width="50"/>フルカス</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="/games/splatoon3/weapons/デンタルワイパーミント/"><img alt="デンタルワイパーミント画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/mint-decavitator.png" width="50"/>デンパミ</a>
</td>
</tr>
<tr>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="デンタルワイパースミ画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/charcoal-decavitator.png" width="50"/>デンスミ</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="/games/splatoon3/weapons/14式竹筒銃-乙/"><img alt="14式竹筒銃・乙画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/bamboozler-14-mk-ii.png" width="50"/>竹乙</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="/games/splatoon3/weapons/24式張替傘-乙/"><img alt="24式張替傘・乙画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/recycled-brella-24-mk-ii.png" width="50"/>和傘乙</a>
</td>
</tr>
<tr>
<td class="center" width="33%">
<a class="a-link" href="/games/splatoon3/weapons/イグザミナーヒュー/"><img alt="イグザミナーヒュー画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/heavy-edit-splatling-nouveau.png" width="50"/>ザミヒュー</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="/games/splatoon3/weapons/ロングブラスターカスタム/"><img alt="ロングブラスターカスタム画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/custom-range-blaster.png" width="50"/>ロンカス</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="/games/splatoon3/weapons/ガエンffカスタム/"><img alt="ガエンFFカスタム画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/custom-douser-dualies-ff.png" width="50"/>ガエカス</a>
</td>
</tr>
<tr>
<td class="center" width="33%">
<a class="a-link" href="/games/splatoon3/weapons/ハイドラントカスタム/"><img alt="ハイドラントカスタム画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapons/custom-hydra-splatling.png" width="50"/>ハイカス</a>
</td>
<td class="center" width="33%">-</td>
<td class="center" width="33%">-</td>
</tr>
</table>
<h3 class="a-header--3" id="hm_20">最強武器ランキング一覧</h3>
<table class="a-table a-table a-table" style="">
<tr>
<td class="center" colspan="5" width="100%"><a class="a-link" href="/games/splatoon3/tier-list/"><img alt="最強" class="a-img img-placeholder" loading="lazy" src="/images/games/splatoon3/placeholder-icon.png" width="440"/>▶︎最強ブキ</a></td>
</tr>
<tr>
<th colspan="5">ルール別最強武器ランキング</th>
</tr>
<tr>
<td class="center" width="20%"><a class="a-link" href="/games/splatoon3/tier-area/"><img alt="スプラトゥーン2のガチエリア" class="a-img" height="100" loading="lazy" src="/images/games/splatoon3/rules/splat-zones.png" width="100"/><br/>ガチ<br/>エリア</a></td>
<td class="center" width="20%"><a class="a-link" href="/games/splatoon3/tier-yagura/"><img alt="スプラトゥーン2のガチヤグラ" class="a-img" height="100" loading="lazy" src="/images/games/splatoon3/rules/tower-control.png" width="100"/><br/>ガチ<br/>ヤグラ</a></td>
<td class="center" width="20%"><a class="a-link" href="/games/splatoon3/tier-hoko/"><img alt="スプラトゥーン2のガチホコ" class="a-img" height="100" loading="lazy" src="/images/games/splatoon3/rules/rainmaker.png" width="100"/><br/>ガチ<br/>ホコ</a></td>
<td class="center" width="20%"><a class="a-link" href="/games/splatoon3/tier-asari/"><img alt="スプラトゥーン2のガチアサリ" class="a-img" height="100" loading="lazy" src="/images/games/splatoon3/rules/clam-blitz.png" width="100"/><br/>ガチ<br/>アサリ</a></td>
<td class="center" width="20%"><a class="a-link" href="/games/splatoon3/tier-nawabari/"><img alt="スプラトゥーン2のナワバリ" class="a-img" height="100" loading="lazy" src="/images/games/splatoon3/rules/turf-war.png" width="100"/><br/>ナワ<br/>バリ</a></td>
</tr>
</table>
<h3 class="a-header--3" id="hm_21">武器の評価一覧</h3>
<table class="a-table a-table table--fixed a-table" style="">
<tr>
<td class="center" colspan="3" width="100%"><a class="a-link" href="#"><img alt="ブキ評価" class="a-img img-placeholder" loading="lazy" src="/images/games/splatoon3/placeholder-icon.png" width="440"/>▶︎武器の評価一覧に戻る</a></td>
</tr>
<tr>
<th class="center" colspan="3" width="100%">ブキ種別評価一覧</th>
</tr>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="シューター画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapon_class/shooter.png" width="50"/>シューター</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="ローラー画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapon_class/roller.png" width="50"/>ローラー</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="チャージャー画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapon_class/charger.png" width="50"/>チャージャー</a>
</td>
<tr>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="スロッシャー画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapon_class/slosher.png" width="50"/>スロッシャー</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="スピナー画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapon_class/splatling.png" width="50"/>スピナー</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="マニューバー画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapon_class/dualie.png" width="50"/>マニューバー</a>
</td>
</tr>
<tr>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="シェルター画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapon_class/brella.png" width="50"/>シェルター</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="ブラスター画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapon_class/blaster.png" width="50"/>ブラスター</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="フデ画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapon_class/brush.png" width="50"/>フデ</a>
</td>
</tr>
<tr>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="ストリンガー画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapon_class/stringer.png" width="50"/>ストリンガー</a>
</td>
<td class="center" width="33%">
<a class="a-link" href="#"><img alt="ワイパー画像" class="a-img" height="50" loading="lazy" src="/images/games/splatoon3/weapon_class/splatana.png" width="50"/>ワイパー</a>
</td>
</tr>
</table>
<h3 class="a-header--3" id="hm_22">ブキ関連人気記事</h3>
<table class="a-table a-table a-table" style="">
<tr>
<td class="center" width="33%"><a class="a-link" href="#"><img alt="射程ランキング・射程表" class="a-img img-placeholder" loading="lazy" src="/images/games/splatoon3/placeholder-icon.png" width="440"/>射程表</a></td>
<td class="center" width="33%"><a class="a-link" href="#"><img alt="武器の入手方法" class="a-img img-placeholder" loading="lazy" src="/images/games/splatoon3/placeholder-icon.png" width="440"/>入手方法</a></td>
<td class="center" width="33%"><a class="a-link" href="#"><img alt="サブスペ" class="a-img img-placeholder" loading="lazy" src="/images/games/splatoon3/placeholder-icon.png" width="440"/>サブスペ一覧</a></td>
</tr>
</table>
